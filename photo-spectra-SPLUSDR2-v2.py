...
mag_PStotal = [[] for _ in range(n)]
mag_aper = [[] for _ in range(n)]

#Error
mag_auto_err  = [[] for _ in range(n)]
mag_PStotal_err  = [[] for _ in range(n)]
...
for i in range(n):
    ...
    #PStotal
    mag_PStotal[i].append(data["U_PStotal"][i])
    mag_PStotal[i].append(data["F378_PStotal"][i])
    mag_PStotal[i].append(data["F395_PStotal"][i])
    mag_PStotal[i].append(data["F410_PStotal"][i])
    mag_PStotal[i].append(data["F430_PStotal"][i])
    mag_PStotal[i].append(data["G_PStotal"][i])
    mag_PStotal[i].append(data["F515_PStotal"][i]) 
    mag_PStotal[i].append(data["R_PStotal"][i]) 
    mag_PStotal[i].append(data["F660_PStotal"][i])
    mag_PStotal[i].append(data["I_PStotal"][i]) 
    mag_PStotal[i].append(data["F861_PStotal"][i]) 
    mag_PStotal[i].append(data["Z_PStotal"][i])
    ...
    #ERRO PStotal
    mag_PStotal_err[i].append(data["e_U_PStotal"][i])
    mag_PStotal_err[i].append(data["e_F378_PStotal"][i])
    mag_PStotal_err[i].append(data["e_F395_PStotal"][i])
    mag_PStotal_err[i].append(data["e_F410_PStotal"][i])
    mag_PStotal_err[i].append(data["e_F430_PStotal"][i])
    mag_PStotal_err[i].append(data["e_G_PStotal"][i])
    mag_PStotal_err[i].append(data["e_F515_PStotal"][i]) 
    mag_PStotal_err[i].append(data["e_R_PStotal"][i]) 
    mag_PStotal_err[i].append(data["e_F660_PStotal"][i])
    mag_PStotal_err[i].append(data["e_I_PStotal"][i]) 
    mag_PStotal_err[i].append(data["e_F861_PStotal"][i]) 
    mag_PStotal_err[i].append(data["e_Z_PStotal"][i])
    ...
    ax1.plot(np.array(wl)[mask_au], np.array(mag_auto[i])[mask_au], '-k', alpha=0.2)#, label='Auto')
    for wl1, mag, mag_err, colors, marker_ in zip(np.array(wl)[mask_au], np.array(mag_auto[i])[mask_au], mag_auto_err[i], color, marker):
        ax1.scatter(wl1, mag, color = colors, marker=marker_, s=600, zorder=10)
        ax1.errorbar(wl1, mag, yerr=mag_err, marker='.', fmt='.', color=colors, ecolor=colors, elinewidth=5.9, markeredgewidth=5.2,  capsize=20)
    # plt.text(0.06, 0.1, "Fr 2-21",
    #          transform=ax.transAxes, fontsize=48,  fontdict=font)
    #plt.subplots_adjust(bottom=0.19)
    plt.legend(fontsize=20.0)
    ...
    if file_.endswith("dat"):
        plotfile = "photopectrum_splus_"+str(data["ID"][i].split("S.")[-1].split(".g")[0]).replace(".", "-")+"_{}_PStotal.pdf".format(file_.split('.da')[0])
    else:
        plotfile = "photopectrum_splus_"+str(data["ID"][i].split("S.")[-1].split(".g")[0]).replace(".", "-")+"_{}_PStotal.pdf".format(file_.split('.tab')[0])
    fig = plt.figure(figsize=(15.5, 9.5))
    ax2 = fig.add_subplot(1,1,1)
    plt.tick_params(axis='x', labelsize=42) 
    plt.tick_params(axis='y', labelsize=42)
    ax2.set_xlim(left=3000, right=9700)
    #ax.set_ylim(ymin=17.5,ymax=23)
    #ax1.set_xlabel(r'$\lambda$')
    ax2.set_xlabel(r'Wavelength $[\mathrm{\AA]}$', fontsize = 44)
    ax2.set_ylabel(r'Magnitude [AB]', fontsize = 44)

    # Mask to no take the values 99.0
    mask_p = [mag_PStotal[i][m] != 99.0 for m in range(len(mag_PStotal[0]))]   

    ax2.plot(np.array(wl)[mask_p], np.array(mag_PStotal[i])[mask_p], '-k', alpha=0.2)#, label='Auto')
    for wl1, mag_1, mag_err_1, colors, marker_ in zip(np.array(wl)[mask_p], np.array(mag_PStotal[i])[mask_p], mag_PStotal_err[i], color, marker):
        ax2.scatter(wl1, mag_1, color = colors, marker=marker_, s=600, zorder=10)
        ax2.errorbar(wl1, mag_1, yerr=mag_err_1, marker='.', fmt='.', color=colors, ecolor=colors, elinewidth=5.9, markeredgewidth=5.2,  capsize=20)
    # plt.text(0.06, 0.1, "Fr 2-21",
    #          transform=ax.transAxes, fontsize=48,  fontdict=font)
    #plt.subplots_adjust(bottom=0.19)
    plt.legend(fontsize=20.0)
