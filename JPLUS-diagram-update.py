...
for file_name in file_list:
    with open(file_name) as f:
        ...
        plot_mag("F910_z_sdss", "F660", "F480_g_sdss")
...
err_zpVale_h4 = [0.061, 0.029, 0.067, 0.036, 0.044, 0.042, 0.043, 0.048, 0.030, 0.092] # z filter uded coadded valera
err_zpVale_pn135 = [0.115, 0.103, 0.127, 0.073, 0.073, 0.073, 0.078, 0.071, 0.070, 0.100 ]
...
for file_name1 in file_list1:
    with open(file_name1) as f1:
        data1 = json.load(f1)
        fil1 = data1["J0911_zSDSS"]-25+22.562#-1.12
        fil2 = data1["J0660"]-25+20.859
        fil3 = data1["J0480_gSDSS"]-25+23.297
        diff = fil1 - fil2
        diff0 = fil1 - fil3
...
for file_name2 in file_list2:
    with open(file_name2) as f2:
        data2 = json.load(f2)
        fil1 = data2["J0911_zSDSS"]-25+22.535#-1.162
        fil2 = data2["J0660"]-25+21.058
        fil3 = data2["J0480_gSDSS"]-25+23.425
        diff = fil1 - fil2
        diff0 = fil1 - fil3
        d_644_jplus.append(diff)
        d_768_jplus.append(diff0)
...
err_Val_h4x= np.sqrt(err_zpVale_h4[9]**2+err_zpVale_h4[3]**2)
err_Val_h4y= np.sqrt(err_zpVale_h4[9]**2+err_zpVale_h4[6]**2)
err_Val_pn135x= np.sqrt(err_zpVale_pn135[9]**2+err_zpVale_pn135[3]**2)
err_Val_pn135y= np.sqrt(err_zpVale_pn135[9]**2+err_zpVale_pn135[6]**2)
...
plt.xlabel(r'$zSDSS - gSDSS$', fontsize= 18)
plt.ylabel(r'$zSDSS - J0660$', fontsize= 18)
...
ax2.errorbar(B1[139], A1[139], yerr=0.3,  lolims= True,  marker='.', fmt='k.', elinewidth=.8, markeredgewidth=.8, markersize=8,)
ax2.errorbar(B1[139], A1[139], xerr=0.25,  xlolims= True,  marker='.', fmt='k.', elinewidth=.8, markeredgewidth=.8, markersize=8,)
...
for label_, x, y in zip(label, B1[0], A1[0]):
    ax2.annotate(label_, (x, y), alpha=10, size=8,
                   xytext=(-3.0, -6.6), textcoords='offset points', ha='right', va='bottom',)

ax2.annotate("H4-1", (np.array(B1[139]), np.array(A1[139])), alpha=10, size=8,
                   xytext=(21, -1.0), textcoords='offset points', ha='right', va='bottom',)
...
plt.savefig('Fig1-JPLUS-J0660-v0.pdf')
plt.clf()
...
ax3.errorbar(B1[139], A1[139], yerr=0.3,  lolims= True,  marker='.', fmt='k.', elinewidth=.8, markeredgewidth=.8, markersize=8,)
ax3.errorbar(B1[139], A1[139], xerr=0.25,  xlolims= True,  marker='.', fmt='k.', elinewidth=.8, markeredgewidth=.8, markersize=8,)
...
ax3.legend(scatterpoints=1, ncol=1, fontsize=7.0, loc='upper left', **lgd_kws)
ax3.grid()
...
plt.tight_layout()
#plt.savefig('luis-JPLUS-Viironen.pdf')#,  bbox_extra_artists=(lgd,), bbox_inches='tight')
plt.savefig('Fig3-JPLUS-J0515-J0660-v0.pdf')
plt.clf()
